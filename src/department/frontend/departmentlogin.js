...
import DeptForgotPassword from './deptforgotpassword'; // Import your forgot password component
  ...
  const loginHandler = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        localStorage.setItem('departmentLoginMessage', 'Login successful'); // <-- Consistent message
        onLoginSuccess();
      } else {
        ...
        if (
          ...
          const deptUser = response.data.user || {};
          const year = deptUser.academic_year || '2024–2025';
      ...
      if (msg && msg.toLowerCase().includes('locked')) {
        const deptUser = err.response?.data?.user || {};
        const year = deptUser.academic_year || '2024–2025';
        setError(`${year} Academic Year details are already finished!`);
        setIsLocked(true);
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center">
      <div className="max-w-md w-full mx-auto px-4">
        {!showForgot ? (
            ...
            <form onSubmit={loginHandler} className="space-y-6">
              ...
              <div className="flex justify-end mt-2">
                <motion.button
                  ...
                  disabled={isSendingPassword || isLoading || isLocked}
          ...
          <DeptForgotPassword onBackToLogin={() => setShowForgot(false)} />
        )}
      </div>
    </div>
  );
