...
export default function DeptWelcomeDashboard() {
  ...
  useEffect(() => {
    // Get department user info from localStorage
    let deptUser = null;
    try {
      deptUser = JSON.parse(localStorage.getItem("departmentUser"));
    } catch {}
    setUserName(deptUser?.name || "Department User");
    setDepartment(deptUser?.department || "");
    ...
    async function fetchAcademicYear() {
      try {
        ...
        if (res.data.success && Array.isArray(res.data.years) && res.data.years.length > 0) {
          setAcademicYear(res.data.years[0]);
        } else {
          setAcademicYear("");
        }
      } catch {
    ...
    if (deptUser?.dept_id) fetchAcademicYear();
          ...
          <div className="flex-1 flex flex-col items-center md:items-start gap-8">
            ...
            <div>
              <h1 className="text-5xl font-extrabold text-gray-900 mb-4 tracking-tight leading-tight">
                Welcome to <span className="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Department Dashboard</span>
              </h1>
              <p className="text-lg text-gray-600 max-w-2xl mb-6">
                Hello <span className="font-bold text-blue-700">{userName}</span>, this is your professional dashboard for managing department data, submissions, and reports for your institution.
              </p>
            </div>
          </div>
