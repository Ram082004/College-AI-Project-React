...
function DepartmentDetailsModel({ deptId }) {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(DEPT_DETAILS_API(deptId), {
      ...
      .then(res => {
        if (res.data.success) {
          ...
          // Default to first allowed degree level
  ...
  // Helper to get year slots for current degree level (used for Enrollment)
  const getYearSlotsForLevel = () => (degreeLevel === 'UG' ? yearSlots.UG : yearSlots.PG);

  // Examination-specific single slot (match studentExamination behavior)
  const getExamYearSlot = () => (degreeLevel === 'UG' ? 'III Year' : 'II Year');

  // Normalization helper: safe lowercase trimmed string
  const norm = v => (v === null || v === undefined) ? '' : String(v).trim().toLowerCase();

  // Filtered details for summary (respect year slot and result type)
  const filteredDetails = (activeTab === 'enrollment' ? enrollmentDetails : examinationDetails)
    .filter(row => {
      if (!row) return false;

      // degree level compare normalized (if provided on row)
      if (row.degree_level && norm(row.degree_level) !== norm(degreeLevel)) return false;

      // academic year filter (if a global academicYear is selected)
      if (academicYear && row.academic_year && norm(row.academic_year) !== norm(academicYear)) return false;

      // Enrollment uses full year slots; Examination uses a single exam slot (III Year for UG, II Year for PG)
      ...
      // examination specific: match result type when activeTab is examination
      if (activeTab === 'examination' && examResultType) {
        // normalize "result_type" if present
        if (row.result_type && norm(row.result_type) !== norm(examResultType)) return false;
      }

      return true;
    });

  // Summary calculation: subcategory -> { male, female, transgender }
  function getSummaryData() {
    const summary = {};
    // initialize entries for known subcategories
    subcategories.forEach(sub => {
      summary[sub] = { male: 0, female: 0, transgender: 0 };
    });

    filteredDetails.forEach(row => {
      // robust detection of subcategory field
      ...
      // robust detection of numeric gender fields (many API shapes)
      const maleVal = Number(row.male ?? row.male_count ?? row.male_students ?? row.m ?? 0) || 0;
      const femaleVal = Number(row.female ?? row.female_count ?? row.female_students ?? row.f ?? 0) || 0;
      const transVal = Number(row.transgender ?? row.transgender_count ?? row.transgender_students ?? row.t ?? 0) || 0;

      // If there are explicit numeric gender fields, use them.
      if (maleVal || femaleVal || transVal) {
        ...
        // fallback: sometimes API returns a single count with a gender field
  ...
  function getSubcategoryBreakdown(subcategory) {
    const breakdownRows = filteredDetails.filter(row => {
    ...
    return categories.map(category => {
      ...
      breakdownRows.forEach(row => {
        const catName = row.category || row.category_name || row.cate || '';
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      <div className="mb-8 flex justify-center">
        <div className="flex items-center gap-3 bg-gradient-to-r from-blue-50 via-teal-50 to-purple-50 border border-teal-200 rounded-2xl px-6 py-3 shadow-lg">
          <span className="text-xs text-gray-500 font-semibold uppercase tracking-wider">Academic Year</span>
          <span className="text-lg font-bold text-teal-700 tracking-wide">{academicYear || <span className="text-gray-400">N/A</span>}</span>
        </div>
      </div>
      ...
      {/* Year slot (enrollment) and result type (examination) controls */}
          ...
          <div className="flex items-center gap-3">
            <div className="flex space-x-2">
              {getYearSlotsForLevel().map((slot, idx) => (
                <button
                  key={slot}
                  className={`px-6 py-2 rounded-lg font-semibold transition-colors border-2 ${
                    currentYearSlot === idx
      ...
      <div className="mb-12">
        <div className="flex flex-col items-center mb-6">
          <h3 className="text-lg font-semibold text-gray-900">
            {activeTab === 'enrollment' ? 'Enrollment Summary' : 'Examination Summary'} ({degreeLevel})
          </h3>
        </div>
                      ...
                      return (
                        <tr>
                          <td colSpan={4} className="py-8 text-center text-gray-400">
                            ...
                            <span>No data available for this subcategory and degree level.</span>
                          </td>
                        </tr>
                      );
                    }
...
export default function DepartmentDetails() {
  ...
  useEffect(() => {
    if (departmentUsers.length > 0) {
      setLatestAcademicYear(departmentUsers[0].academic_year || "");
    }
  }, [departmentUsers]);

  // Get unique department names and ids
  const departments = Array.from(
    new Set(departmentUsers.map(u => JSON.stringify({ id: u.dept_id, name: u.department })))
  ).map(str => JSON.parse(str)).filter(d => d.name);
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mb-10">
        {departments.length === 0 ? (
          ...
          departments.map(dept => (
            <button
              key={dept.id}
              className={`relative bg-gradient-to-br from-white via-blue-50 to-teal-50 p-6 rounded-3xl shadow-xl border-2 flex flex-col items-center cursor-pointer transition-all duration-200 ${
                selectedDept && selectedDept.id === dept.id
                  ? 'border-teal-500 ring-2 ring-teal-300 scale-105'
                  : 'border-gray-100 hover:border-teal-400'
              }`}
            ...
            >
              ...
              {selectedDept && selectedDept.id === dept.id && (
                <span className="absolute top-3 right-3 bg-teal-500 text-white rounded-full w-6 h-6 flex items-center justify-center shadow">
                  ✓
                </span>
              )}
            </button>
      ...
      {selectedDept && (
        <DepartmentDetailsModel deptId={selectedDept.id} />
      )}
    </div>
  );
}