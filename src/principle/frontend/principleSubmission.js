...
function PrincipleSubmission() {
  ...
  // Pagination logic
  const totalPages = Math.ceil(submittedData.length / recordsPerPage);
  const paginatedRows = submittedData.slice(
    ...
    return (
      <div className="flex justify-center items-center gap-2 mt-6">
        ...
        {Array.from({ length: totalPages }, (_, i) => (
          <button
            key={i + 1}
            className={`px-3 py-1 rounded font-semibold ${currentPage === i + 1 ? "bg-blue-600 text-white" : "bg-blue-50 text-blue-700"}`}
