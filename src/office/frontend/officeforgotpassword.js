...
export default function OfficeForgotPassword({ onBackToLogin }) {
  ...
  // OTP handlers (auto-advance, paste, backspace)
  const handleOtpInput = (index, value) => {
    ...
    if (digit) {
      ...
      setOtp(arr.join(''));
      const next = index + 1;
      if (next < 6 && otpRefs.current[next]) otpRefs.current[next].focus();
    } else {
      // user cleared input (or non-digit)
      arr[index] = '';
      setOtp(arr.join(''));
    }
  };
  ...
  const handleOtpKeyDown = (index, e) => {
    if (e.key === 'Backspace') {
      if ((otp[index] || '').length === 0 && index > 0) {
        const prev = index - 1;
        if (otpRefs.current[prev]) otpRefs.current[prev].focus();
      }
  ...
  const sendOtp = async () => {
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/office/office-forgot-password`,
  ...
  const verifyOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await axios.post(`${API_BASE_URL}/office/office-verify-otp`, { email: email.trim().toLowerCase(), otp });
      if (res.data.success) {
        setStep(3);
  ...
  const resetPassword = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await axios.post(`${API_BASE_URL}/office/office-reset-password`, { email: email.trim().toLowerCase(), otp, newPassword });
      if (res.data.success) {
              ...
              {step === 1 && (
                <motion.div>
                  <input
                    ...
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-400 text-white placeholder-gray-400"
                  />
                  <motion.button
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        >
                          {showNewPassword ? <RiEyeOffLine style={{ color: 'white' }} /> : <RiEyeLine style={{ color: 'white' }} />}
                        </button>
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <button
                        ...
                        className="absolute right-2 top-2"
                      >
                        {showConfirmPassword ? <RiEyeOffLine style={{ color: 'white' }} /> : <RiEyeLine style={{ color: 'white' }} />}
                      </button>
                    </div>
                  </div>
                  <motion.button
